import { groupBy, sortBy, difference, pullAll } from 'lodash';
import fs from 'fs';
...
function roadsClosestTo(roads: Road[], road: Road): Road[] {
  return [...roads].sort((a, b) => distanceBetweenRoads(a, road) - distanceBetweenRoads(b, road)).slice(1);
}
...
function getRoute(roads: Road[]): { route: Road[]; remainingRoads: Road[] } {
  ...
  while (cost < TARGET_COST && remainingRoads.length) {
    const road = pickRoadCloseToTargetCost(remainingRoads, TARGET_COST - cost);
    cost += road.cost;
...
function pickRoadCloseToTargetCost(roads: Road[], targetCost: number): Road {
  return sortBy(roads, (r) => Math.abs(r.cost - targetCost))[0];
}
